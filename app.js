...
// ============================================================================
    ...
    window.openFacultiesList = function() {
        ...
        listHtml += `</div>`;
        window.openModal('🏛️ Fakülte Seçimi', listHtml);
    };
    ...
    window.checkFacultyPasscode = async function(facName) {
        ...
        let expectedCode = firstWord.substring(0, 3) + "1000"; 
    ...
    function setupGroupChatListener(roomId) {
        currentGroupUnsubscribe = onSnapshot(doc(db, "group_chats", roomId), (docSnap) => {
            ...
            let chatHTML = `<div style="text-align:center; padding:10px; color:#6B7280; font-size:11px; font-weight:600; align-self:center;">Bugün</div>`;

            msgs.forEach(msg => {
                if (msg.isSystem) {
                    chatHTML += `
                        <div style="text-align:center; margin: 12px 0; align-self:center;">
                ...
                chatHTML += `
                    <div class="bubble ${type}" style="display:flex; flex-direction:column;" onclick="let btn=this.querySelector('.admin-del-btn'); if(btn) { btn.style.display = btn.style.display === 'none' ? 'flex' : 'none'; }">
                        ...
                        ${mediaHtml}
                        ${textHtml}
                        <div class="msg-time" style="align-self:flex-end; font-size:10px; margin-top:4px; font-weight:500;">${msg.time}</div>
                    </div>
                `;
            });
    ...
    window.initEmbeddedFastMatch = async function() {
        ...
        if (count >= maxSwipes) {
            let limitMessage = window.userProfile.isPremium 
                ? "Premium eşleşme limitine ulaştın. Eğlenceye devam et!"
                : "Günlük eşleşme hakkın doldu. Biraz kafa dağıtmaya ne dersin?";
                
            let actionButton = `<button class="btn-primary" style="width:100%; justify-content:center; background:linear-gradient(135deg, #111827, #374151); color:white; border:none; padding:14px; border-radius:12px; font-weight:bold; font-size:15px; margin-top:10px; box-shadow:0 4px 6px rgba(0,0,0,0.3);" onclick="window.startCampusRunGame()">🏃‍♂️ Kampüste Yarış</button>`;
            ...
            container.innerHTML = `
                <div style="text-align:center; padding:20px; background:white; border-radius:16px; box-shadow:0 4px 6px rgba(0,0,0,0.05); width:100%; max-width:320px;">
                    ...
                    <p style="color:var(--text-gray); font-size:13px; margin-bottom:20px;">${limitMessage}</p>
                    ${actionButton}
                </div>
            `;
            return;
    ...
    window.startCampusRunGame = function() {
        const mainArea = document.getElementById('main-content');
        const headerRightMenu = document.querySelector('.header-right-menu');
        
        if (headerRightMenu && !document.getElementById('game-exit-btn')) {
            const exitBtnHtml = `<div id="game-exit-btn" onclick="window.stopCampusRunGame()" title="Oyundan Çık" style="width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; background:white; border:1px solid #111827; margin-right:8px; box-sizing:border-box;"><svg viewBox="0 0 24 24" width="20" height="20" stroke="#111827" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></div>`;
            headerRightMenu.insertAdjacentHTML('afterbegin', exitBtnHtml);
        }

        mainArea.innerHTML = `<div style="width:100%; height:100%; background:#050d1f; overflow:hidden;"><iframe id="game-iframe" style="width:100%; height:100%; border:none; border-radius:0;"></iframe></div>`;

        const iframe = document.getElementById('game-iframe');
        
        const gameSource = `<!DOCTYPE html>
<html lang="tr">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
<title>Derse Yetiş!</title>
<style>
...
:root{
  --navy:#050d1f;--deep:#0a1628;--mid:#0e2040;
  --cyan:#00d4ff;--cyan2:#00a8cc;
  --gold:#ffc300;--gold2:#e6a800;
  --red:#ff3a3a;--green:#00ff88;--white:#f0f4ff;
}
...
box-shadow:0 0 0 1px rgba(0,212,255,.15),0 0 40px rgba(0,212,255,.12),0 0 100px rgba(0,80,160,.25);
}
...
.hud-label{font-family:'Orbitron',monospace;font-size:9px;letter-spacing:3px;color:rgba(0,212,255,.55);text-transform:uppercase;}
.hud-value{font-family:'Orbitron',monospace;font-size:26px;font-weight:900;color:var(--white);
text-shadow:0 0 14px rgba(0,212,255,.7),0 0 28px rgba(0,212,255,.3);}
#hearts{display:flex;gap:6px;align-items:center;padding-top:4px;}
.heart-icon{font-size:20px;filter:drop-shadow(0 0 4px #ff3a3a);transition:filter .2s;}
.heart-dead{font-size:20px;opacity:.2;}

#speedBar{
position:absolute;bottom:18px;left:50%;transform:translateX(-50%);
font-family:'Orbitron',monospace;font-size:10px;letter-spacing:2px;
color:var(--gold);text-shadow:0 0 8px var(--gold2);
...
overflow-y:auto;padding:40px 28px 32px;
}
.screen.off{display:none!important;}
.screen-bg{
position:absolute;inset:0;
background:linear-gradient(160deg,rgba(5,13,31,.97),rgba(10,22,40,.98),rgba(14,32,64,.95));
backdrop-filter:blur(4px);
}
.screen-bg::after{
content:'';position:absolute;inset:0;
background:linear-gradient(rgba(0,212,255,.035) 1px,transparent 1px),
linear-gradient(90deg,rgba(0,212,255,.035) 1px,transparent 1px);
background-size:40px 40px;
}
.s-inner{position:relative;z-index:2;width:100%;display:flex;flex-direction:column;align-items:center;}

.logo{
width:68px;height:68px;margin-bottom:18px;
border-radius:18px;display:flex;align-items:center;justify-content:center;
font-size:30px;background:linear-gradient(135deg,#0066cc,#00aaff);
box-shadow:0 0 28px rgba(0,150,255,.45);
}
.eyebrow{font-family:'Orbitron',monospace;font-size:10px;letter-spacing:5px;
color:rgba(0,212,255,.55);text-transform:uppercase;margin-bottom:10px;}
.stitle{font-family:'Orbitron',monospace;font-size:40px;font-weight:900;color:var(--white);
text-align:center;line-height:1.1;margin-bottom:6px;
text-shadow:0 0 24px rgba(0,212,255,.45);}
.ssub{font-size:13px;color:rgba(240,244,255,.38);text-align:center;margin-bottom:30px;
line-height:1.7;letter-spacing:.4px;}

.final-n{font-family:'Orbitron',monospace;font-size:58px;font-weight:900;
color:var(--cyan);text-shadow:0 0 24px rgba(0,212,255,.8);margin-bottom:4px;}
.final-l{font-size:10px;letter-spacing:4px;color:rgba(255,255,255,.3);margin-bottom:26px;}

.divider{width:100%;height:1px;background:linear-gradient(90deg,transparent,rgba(0,212,255,.22),transparent);margin:4px 0 20px;}

input#pname{
width:100%;padding:14px 18px;margin-bottom:14px;
background:rgba(0,212,255,.05);border:1px solid rgba(0,212,255,.22);
border-radius:10px;color:var(--white);font-size:17px;font-weight:600;
font-family:'Exo 2',sans-serif;text-align:center;letter-spacing:2px;outline:none;
transition:border-color .2s,box-shadow .2s;
}
input#pname::placeholder{color:rgba(255,255,255,.18);}
input#pname:focus{border-color:rgba(0,212,255,.65);box-shadow:0 0 20px rgba(0,212,255,.12);}
...
.btn-p{background:linear-gradient(135deg,#0066cc,#00aaff);color:#fff;
box-shadow:0 4px 24px rgba(0,150,255,.32),inset 0 1px 0 rgba(255,255,255,.15);}
.btn-p:hover{box-shadow:0 6px 32px rgba(0,150,255,.5);}
.btn-g{background:linear-gradient(135deg,var(--gold2),var(--gold));color:#0a1628;
box-shadow:0 4px 20px rgba(255,195,0,.28);}
.btn-ghost{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);
color:rgba(240,244,255,.55);box-shadow:none;}

#lbBox,#lbBox2{width:100%;margin-top:12px;}
.lb-t{font-family:'Orbitron',monospace;font-size:9px;letter-spacing:3px;
color:rgba(0,212,255,.5);text-align:center;margin-bottom:10px;}
.lb-r{display:flex;align-items:center;padding:10px 12px;border-radius:8px;
margin-bottom:4px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);}
.lb-r.g1{background:rgba(255,195,0,.07);border-color:rgba(255,195,0,.2);}
.lb-r.g2{background:rgba(200,200,210,.06);border-color:rgba(200,200,210,.15);}
.lb-r.g3{background:rgba(205,120,50,.07);border-color:rgba(205,120,50,.15);}
.lb-pos{width:32px;font-size:14px;font-weight:700;color:rgba(255,255,255,.28);}
.g1 .lb-pos{color:var(--gold);}.g2 .lb-pos{color:#c8c8d2;}.g3 .lb-pos{color:#cd7832;}
.lb-nm{flex:1;font-size:15px;font-weight:600;color:var(--white);letter-spacing:.5px;}
.lb-sc{font-family:'Orbitron',monospace;font-size:13px;color:var(--cyan);}

#hint{
position:absolute;bottom:20px;left:50%;transform:translateX(-50%);
display:flex;align-items:center;gap:12px;
font-size:11px;letter-spacing:3px;color:rgba(255,255,255,.18);
pointer-events:none;z-index:9;
}
.ha{animation:hp 1.8s ease-in-out infinite;}
.ha:last-child{animation-delay:.4s;}
@keyframes hp{0%,100%{opacity:.18;}50%{opacity:.55;}}

.corner{position:absolute;width:16px;height:16px;pointer-events:none;}
.ctl{top:10px;left:10px;border-top:2px solid rgba(0,212,255,.3);border-left:2px solid rgba(0,212,255,.3);}
.ctr{top:10px;right:10px;border-top:2px solid rgba(0,212,255,.3);border-right:2px solid rgba(0,212,255,.3);}
.cbl{bottom:10px;left:10px;border-bottom:2px solid rgba(0,212,255,.3);border-left:2px solid rgba(0,212,255,.3);}
.cbr{bottom:10px;right:10px;border-bottom:2px solid rgba(0,212,255,.3);border-right:2px solid rgba(0,212,255,.3);}
</style>
...
<div id="shell">
  ...
  <div id="hud">
    <div class="hud-box">
      ...
      <span class="hud-value" id="sv">0<small style="font-size:14px;opacity:.55">m</small></span>
    </div>
    <div id="hearts">
  ...
  <div id="speedBar">⚡ HIZ ×<span id="spd">1.0</span></div>
  <div id="hint"><span class="ha">◀</span><span>KAYDIR</span><span class="ha">▶</span></div>
  ...
  <div class="screen" id="menuScreen">
    ...
    <div class="s-inner">
      <div class="logo">🎓</div>
      <div class="eyebrow">Kampüs Koşusu</div>
      <div class="stitle">DERSE<br>YETİŞ!</div>
      <div class="ssub">Engelleri aş, rekoru kır.<br>Heykellere çarpma — derse yetiş!</div>
      <div class="divider"></div>
      <button class="btn btn-p" id="btnStart">▶&nbsp;&nbsp;KOŞMAYA BAŞLA</button>
      <button class="btn btn-ghost" id="btnLB1">🏆&nbsp;&nbsp;Lider Tablosu</button>
      <div id="lbBox"></div>
    </div>
  </div>

  <div class="screen off" id="goScreen">
    <div class="screen-bg"></div>
    <div class="s-inner">
      <div class="logo" style="background:linear-gradient(135deg,#aa1100,#ff4400)">💥</div>
      <div class="eyebrow">Oyun Bitti</div>
      <div class="stitle" style="font-size:28px;margin-bottom:18px">DERSE<br>GEÇ KALDIN!</div>
      <div class="final-n" id="gsFinal">0m</div>
      <div class="final-l">KOŞTUĞUN MESAFE</div>
      <div class="divider"></div>
      <input type="text" id="pname" placeholder="İSMİNİ YAZ..." maxlength="16" autocomplete="off">
      <button class="btn btn-g" id="btnSave">💾&nbsp;&nbsp;KAYDET &amp; TEKRAR OYNA</button>
      <button class="btn btn-ghost" id="btnLB2">🏆&nbsp;&nbsp;Lider Tablosu</button>
      <div id="lbBox2"></div>
    </div>
  </div>
</div>
...
// ─── CONSTANTS ───────────────────────────────────────────────────
function LANES(){ return [W*0.22, W*0.50, W*0.78]; }
const HZ=()=>H*0.38;      
const PLY=()=>H*0.80;     

const OBS_TYPES=[
  {id:'cone',   c1:'#ff7700',c2:'#cc4400',c3:'#ff9933'},
  {id:'pillar', c1:'#4488ff',c2:'#2244bb',c3:'#88aaff'},
  {id:'barrier',c1:'#ff2222',c2:'#aa0000',c3:'#ff6666'},
  {id:'column', c1:'#aabbcc',c2:'#667788',c3:'#ccdde8'},
  {id:'sign',   c1:'#00bb77',c2:'#007744',c3:'#00ffaa'},
  {id:'statue', c1:'#ccaa66',c2:'#8a6030',c3:'#e8cc88'},
  {id:'hydrant',c1:'#ff4444',c2:'#cc1111',c3:'#ffaaaa'},
  {id:'bollard',c1:'#ffbb00',c2:'#cc8800',c3:'#ffe066'},
];

// ─── STATE ───────────────────────────────────────────────────────
let G={},running=false,raf=null;
function newG(){
  return{
    score:0,lives:3,speed:5.5,
    frame:0,scroll:0,
    lane:1,playerX:W*0.5,
    obs:[],obsTimer:0,obsInterval:72,
    inv:0,
    particles:[],
    stars:[],
    shakeT:0,shakeA:0,
...
// ─── LEADERBOARD ─────────────────────────────────────────────────
function getLB(){try{return JSON.parse(localStorage.getItem('dy_lb')||'[]');}catch{return[];}}
function saveLB(name,score){
  let lb=getLB();
  lb.push({name:name.trim()||'Anonim',score});
  lb.sort((a,b)=>b.score-a.score);
  localStorage.setItem('dy_lb',JSON.stringify(lb.slice(0,20)));
}
function renderLB(id){
  const el=document.getElementById(id);
  const lb=getLB();
  if(!lb.length){el.innerHTML='<div style="text-align:center;color:rgba(255,255,255,.22);padding:20px;font-size:12px;letter-spacing:1px">Henüz kayıt yok!</div>';return;}
  const md=['🥇','🥈','🥉'],cl=['g1','g2','g3'];
  el.innerHTML='<div class="lb-t">— LİDER TABLOSU —</div>'+
    lb.map((e,i)=>\`<div class="lb-r \${cl[i]||''}">
      <div class="lb-pos">\${md[i]||(i+1)+'.'}</div>
      <div class="lb-nm">\${e.name}</div>
      <div class="lb-sc">\${e.score}m</div>
    </div>\`).join('');
}

// ─── PERSPECTIVE ─────────────────────────────────────────────────
function persp(wx,depth){
  const cx=W*0.5, hz=HZ();
  const x=cx+(wx-cx)*depth;
  const y=hz+(PLY()-hz)*depth;
  const sc=Math.pow(depth,0.82)*0.92+0.05;
  return{x,y,sc};
}
...
function initStars(){
  G.stars=Array.from({length:70},()=>({
    x:Math.random()*W,y:Math.random()*HZ()*0.88,
    r:Math.random()*1.5+0.3,a:Math.random()*0.55+0.15,
    sp:Math.random()*4+1.5,
  }));
}
...
function drawBG(){
  ...
  sky.addColorStop(0.5,'#0a1e3a');
  sky.addColorStop(1,'#122850');
  ctx.fillStyle=sky; ctx.fillRect(0,0,W,hz+50);

  G.stars.forEach(s=>{
    const tw=0.5+0.5*Math.sin(G.frame*0.02/s.sp);
    ctx.save();
  ...
  ctx.save();
  const mg=ctx.createRadialGradient(W*0.82,hz*0.2,0,W*0.82,hz*0.2,30);
  mg.addColorStop(0,'rgba(255,248,220,0.9)');
  mg.addColorStop(0.5,'rgba(230,200,80,0.4)');
  mg.addColorStop(1,'rgba(230,200,80,0)');
  ctx.fillStyle=mg;
  ctx.beginPath(); ctx.arc(W*0.82,hz*0.2,30,0,Math.PI*2); ctx.fill();
  ctx.globalAlpha=0.7;
  ctx.fillStyle='#fffbe0';
  ctx.beginPath(); ctx.arc(W*0.82,hz*0.2,13,0,Math.PI*2); ctx.fill();
  ctx.restore();

  drawSilhouette(hz);

  const hg=ctx.createLinearGradient(0,hz-40,0,hz+60);
  hg.addColorStop(0,'rgba(0,30,80,0)');
  hg.addColorStop(0.45,'rgba(0,50,120,0.55)');
  hg.addColorStop(1,'rgba(0,0,0,0)');
  ctx.fillStyle=hg; ctx.fillRect(0,hz-40,W,100);

  const road=ctx.createLinearGradient(0,hz,0,H);
  road.addColorStop(0,'#172035');
  road.addColorStop(0.3,'#0f1828');
  road.addColorStop(0.75,'#080e1a');
  road.addColorStop(1,'#030710');
  ctx.fillStyle=road;
  ctx.beginPath();
  ctx.moveTo(W*0.27,hz); ctx.lineTo(W*0.73,hz);
  ...
  ctx.beginPath(); ctx.moveTo(W*0.27,hz); ctx.lineTo(-W*0.2,H); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(W*0.73,hz); ctx.lineTo(W*1.2,H); ctx.stroke();
  ctx.restore();

  const doff=(G.scroll*G.speed*2.2)%80;
  ctx.save();
  ...
  ctx.beginPath(); ctx.moveTo(W*0.5,hz+2); ctx.lineTo(W*0.5,H); ctx.stroke();

  ctx.strokeStyle='rgba(0,200,255,0.14)';
  ctx.lineWidth=1.5; ctx.shadowBlur=0;
  ctx.beginPath(); ctx.moveTo(W*0.37,hz+2); ctx.lineTo(W*0.26,H); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(W*0.63,hz+2); ctx.lineTo(W*0.74,H); ctx.stroke();
  ctx.setLineDash([]); ctx.restore();

  const rg=ctx.createRadialGradient(W*0.5,H,0,W*0.5,H,W*0.8);
  rg.addColorStop(0,'rgba(0,60,140,0.14)'); rg.addColorStop(1,'rgba(0,0,0,0)');
  ctx.fillStyle=rg; ctx.fillRect(0,hz,W,H-hz);
}

function drawSilhouette(hz){
  ctx.save(); ctx.globalAlpha=0.5;
  const blds=[
    {x:0,   w:52, h:58,fl:4,tw:false},
    {x:45,  w:25, h:88,fl:6,tw:true},
    {x:65,  w:58, h:42,fl:3,tw:false},
    {x:118, w:18, h:72,fl:5,tw:true},
    {x:132, w:44, h:36,fl:3,tw:false},
    {x:170, w:22, h:55,fl:4,tw:false},
    {x:W-52,  w:52, h:58,fl:4,tw:false},
    {x:W-90,  w:28, h:92,fl:7,tw:true},
    {x:W-148, w:55, h:46,fl:3,tw:false},
    {x:W-178, w:20, h:75,fl:5,tw:true},
    {x:W-218, w:42, h:38,fl:3,tw:false},
    {x:W-252, w:24, h:58,fl:4,tw:false},
  ];
  blds.forEach(b=>{
    const by=hz-b.h+4,bx=b.x;
    const bg=ctx.createLinearGradient(bx,by,bx+b.w,hz);
    bg.addColorStop(0,'#162440'); bg.addColorStop(1,'#0c1828');
    ctx.fillStyle=bg; ctx.fillRect(bx,by,b.w,b.h);
    const cols=Math.max(2,Math.floor(b.w/11));
    const ww=(b.w-6)/cols-2, wh=(b.h-6)/b.fl-3;
    for(let r=0;r<b.fl;r++) for(let c=0;c<cols;c++){
      const lit=Math.random()>.38;
      const wx=bx+3+c*(ww+2), wy=by+3+r*(wh+3);
      ctx.fillStyle=lit?(Math.random()>.65?'rgba(255,220,80,0.6)':'rgba(0,212,255,0.65)'):'rgba(0,212,255,0.07)';
      ctx.fillRect(wx,wy,ww,wh);
    }
    if(b.tw){
      ctx.fillStyle='rgba(0,212,255,0.45)'; ctx.fillRect(bx+b.w*0.46,by-14,3,14);
      ctx.fillStyle='rgba(255,60,60,0.8)';
      ctx.beginPath(); ctx.arc(bx+b.w*0.5,by-14,4,0,Math.PI*2); ctx.fill();
    }
  });
  ctx.restore();
}
...
function drawPlayer(){
  const px=G.playerX, py=PLY();
  const fr=G.frame;
  if(G.inv>0 && Math.floor(fr/4)%2===1) return;

  const leg=Math.sin(fr*0.34)*24;
  const arm=Math.sin(fr*0.34)*20;
  const bob=Math.sin(fr*0.68)*3.5;
  const lean=Math.sin(fr*0.34)*1.5;
  ...
  const shg=ctx.createRadialGradient(0,4,0,0,4,42);
  shg.addColorStop(0,'rgba(0,0,0,0.8)'); shg.addColorStop(1,'rgba(0,0,0,0)');
  ctx.fillStyle=shg; ctx.scale(1.7,0.35);
  ctx.beginPath(); ctx.arc(0,0,42,0,Math.PI*2); ctx.fill();
  ctx.restore();

  ctx.save();
  ctx.translate(-11, 0);
  ctx.rotate(leg*Math.PI/180*0.7);
  ctx.fillStyle='#1c2540'; ctx.fillRect(-7,-4,14,36);
  ctx.fillStyle='#151c30'; ctx.fillRect(-6,30,12,34);
  ctx.fillStyle='rgba(0,212,255,0.3)'; ctx.fillRect(-6,58,12,6);
  const sk1=ctx.createLinearGradient(-12,62,12,76);
  sk1.addColorStop(0,'#cc1a1a'); sk1.addColorStop(1,'#881010');
  ctx.fillStyle=sk1;
  ctx.beginPath(); ctx.roundRect(-13,62,26,14,[2,2,6,6]); ctx.fill();
  ctx.fillStyle='rgba(255,255,255,0.22)'; ctx.fillRect(-11,65,18,3);
  ctx.fillStyle='rgba(255,255,255,0.6)'; ctx.fillRect(-13,75,26,3);
  ctx.restore();
  ...
  ctx.fillStyle='#151c30'; ctx.fillRect(-6,30,12,34);
  ctx.fillStyle='rgba(0,212,255,0.3)'; ctx.fillRect(-6,58,12,6);
  const sk2=ctx.createLinearGradient(-12,62,12,76);
  sk2.addColorStop(0,'#cc1a1a'); sk2.addColorStop(1,'#881010');
  ctx.fillStyle=sk2;
  ctx.beginPath(); ctx.roundRect(-13,62,26,14,[2,2,6,6]); ctx.fill();
  ctx.fillStyle='rgba(255,255,255,0.22)'; ctx.fillRect(-11,65,18,3);
  ctx.fillStyle='rgba(255,255,255,0.6)'; ctx.fillRect(-13,75,26,3);
  ...
  const jg=ctx.createLinearGradient(-24,-68,24,0);
  jg.addColorStop(0,'#1255aa'); jg.addColorStop(0.5,'#1a70d0'); jg.addColorStop(1,'#0d3d80');
  ctx.fillStyle=jg;
  ctx.beginPath(); ctx.roundRect(-24,-68,48,70,[3,3,8,8]); ctx.fill();

  ctx.fillStyle='rgba(0,212,255,0.16)';
  ctx.fillRect(-24,-68,8,70);
  ctx.fillRect(16,-68,8,70);

  ctx.fillStyle='rgba(255,195,0,0.12)';
  ctx.beginPath(); ctx.arc(0,-34,14,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='rgba(255,195,0,0.8)';
  ctx.font='bold 11px Orbitron,monospace';
  ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillText('DY',0,-34);

  const bpg=ctx.createLinearGradient(-17,-65,17,0);
  bpg.addColorStop(0,'#091e52'); bpg.addColorStop(1,'#060f2a');
  ctx.fillStyle=bpg;
  ctx.beginPath(); ctx.roundRect(-16,-63,32,46,[4]); ctx.fill();
  ctx.strokeStyle='rgba(0,212,255,0.28)';
  ctx.lineWidth=4.5; ctx.lineCap='round';
  ctx.beginPath(); ctx.moveTo(-10,-63); ctx.lineTo(-10,-10); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(10,-63); ctx.lineTo(10,-10); ctx.stroke();
  ctx.strokeStyle='rgba(0,212,255,0.12)';
  ctx.lineWidth=1;
  ctx.beginPath(); ctx.roundRect(-13,-30,26,20,[3]); ctx.stroke();

  ctx.lineCap='round'; ctx.lineWidth=13;
  ctx.save(); ctx.translate(-24,-52); ctx.rotate(arm*Math.PI/180*0.9);
  ctx.strokeStyle='#1a6dd0';
  ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(-16,28); ctx.stroke();
  ctx.fillStyle='#e0a868'; ctx.beginPath(); ctx.arc(-16,28,6,0,Math.PI*2); ctx.fill();
  ctx.restore();
  ctx.save(); ctx.translate(24,-52); ctx.rotate(-arm*Math.PI/180*0.9);
  ctx.strokeStyle='#1a6dd0';
  ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(16,28); ctx.stroke();
  ...
  ctx.fillStyle='#c89050'; ctx.fillRect(-7,0,14,16);
  const hg2=ctx.createRadialGradient(-3,-12,0,0,-12,24);
  hg2.addColorStop(0,'#f5c870'); hg2.addColorStop(1,'#c08040');
  ctx.fillStyle=hg2;
  ctx.beginPath(); ctx.ellipse(0,-10,20,24,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#18080a';
  ctx.beginPath(); ctx.ellipse(0,-24,20,14,-0.05,Math.PI,0); ctx.fill();
  ctx.beginPath(); ctx.arc(-20,-14,6,-0.4,1.2); ctx.fill();
  ctx.beginPath(); ctx.arc(20,-14,6,Math.PI-1.2,Math.PI+0.4); ctx.fill();
  ctx.fillStyle='#c08040';
  ctx.beginPath(); ctx.ellipse(-20,-10,5.5,7.5,0,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(20,-10,5.5,7.5,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#1255aa';
  ctx.beginPath(); ctx.roundRect(-11,12,22,8,[4,4,0,0]); ctx.fill();
  ctx.restore();
...
function drawObs(ob){
  const{x,y,sc}=persp(ob.wx,ob.depth);
  ob.sx=x; ob.sy=y; ob.ssc=sc;
  const W_=60*sc, H_=110*sc;
  ctx.save(); ctx.translate(x,y);
  ctx.save(); ctx.globalAlpha=0.22*Math.min(1,sc*1.2);
  const sg=ctx.createRadialGradient(0,2,0,0,2,W_*0.9);
  sg.addColorStop(0,'rgba(0,0,0,0.85)'); sg.addColorStop(1,'rgba(0,0,0,0)');
  ctx.fillStyle=sg; ctx.scale(1.8,0.38);
  ctx.beginPath(); ctx.arc(0,0,W_*0.9,0,Math.PI*2); ctx.fill();
  ctx.restore();

  const t=ob.type;
  switch(t.id){
    case 'cone':    oCone(W_,H_,t,sc); break;
    case 'pillar':  oPillar(W_,H_,t,sc); break;
    case 'barrier': oBarrier(W_,H_,t,sc); break;
    case 'column':  oColumn(W_,H_,t,sc); break;
    case 'sign':    oSign(W_,H_,t,sc); break;
    case 'statue':  oStatue(W_,H_,t,sc); break;
    case 'hydrant': oHydrant(W_,H_,t,sc); break;
    case 'bollard': oBollard(W_,H_,t,sc); break;
  }
  ctx.restore();
}

function oCone(W_,H_,t,sc){
  const g=ctx.createLinearGradient(-W_/2,-H_,W_/2,0);
  g.addColorStop(0,'#ffaa00'); g.addColorStop(0.4,t.c1); g.addColorStop(1,t.c2);
  ctx.fillStyle=g;
  ctx.beginPath(); ctx.moveTo(0,-H_); ctx.lineTo(W_/2,0); ctx.lineTo(-W_/2,0); ctx.closePath(); ctx.fill();
  [0.28,0.52,0.72].forEach(f=>{
    const bw=W_*f, by=-H_*(1-f);
    ctx.fillStyle='rgba(255,255,255,0.32)'; ctx.fillRect(-bw/2,by,bw,H_*0.05);
  });
  const bg2=ctx.createLinearGradient(-W_*0.6,0,W_*0.6,0);
  bg2.addColorStop(0,'#2a2a2a'); bg2.addColorStop(0.5,'#555'); bg2.addColorStop(1,'#2a2a2a');
  ctx.fillStyle=bg2;
  ctx.beginPath(); ctx.ellipse(0,0,W_*0.56,H_*0.065,0,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='rgba(255,255,255,0.85)';
  ctx.beginPath(); ctx.arc(0,-H_*0.94,W_*0.07,0,Math.PI*2); ctx.fill();
}

function oPillar(W_,H_,t,sc){
  const g=ctx.createLinearGradient(-W_/2,0,W_/2,0);
  g.addColorStop(0,t.c2); g.addColorStop(0.35,t.c3); g.addColorStop(0.65,t.c1); g.addColorStop(1,t.c2);
  ctx.fillStyle=g;
  ctx.beginPath(); ctx.roundRect(-W_*0.36,-H_,W_*0.72,H_,[4,4,0,0]); ctx.fill();
  for(let i=-1;i<=1;i+=1){
    ctx.fillStyle='rgba(0,0,0,0.12)';
    ctx.fillRect(i*W_*0.22-W_*0.04,-H_,W_*0.08,H_);
  }
  ctx.fillStyle=t.c3;
  ctx.beginPath(); ctx.roundRect(-W_*0.55,-H_-W_*0.14,W_*1.1,W_*0.14,[3]); ctx.fill();
  ctx.fillStyle=t.c1;
  ctx.beginPath(); ctx.roundRect(-W_*0.45,-H_-W_*0.25,W_*0.9,W_*0.12,[2]); ctx.fill();
  ctx.save();
  ctx.shadowColor=t.c1; ctx.shadowBlur=16*sc;
  ctx.fillStyle='rgba(80,120,255,0.25)';
  ctx.beginPath(); ctx.arc(0,-H_,W_*0.22,0,Math.PI*2); ctx.fill();
  ctx.restore();
  ctx.fillStyle='#223'; ctx.beginPath(); ctx.roundRect(-W_*0.55,0,W_*1.1,H_*0.06,[2]); ctx.fill();
}

function oBarrier(W_,H_,t,sc){
  const bH=H_*0.42;
  ['#888','#aaa'].forEach((c,i)=>{
    const px=(i===0?-W_*0.38:W_*0.38);
    const pg=ctx.createLinearGradient(px-W_*0.07,0,px+W_*0.07,0);
    pg.addColorStop(0,'#666'); pg.addColorStop(0.5,c); pg.addColorStop(1,'#666');
    ctx.fillStyle=pg; ctx.fillRect(px-W_*0.07,-bH,W_*0.14,bH+bH*0.08);
    ctx.fillStyle='#ccc'; ctx.beginPath(); ctx.ellipse(px,-bH,W_*0.09,W_*0.05,0,0,Math.PI*2); ctx.fill();
  });
  [[0.15,t.c1,t.c2],[0.45,'#eee','#bbb'],[0.78,t.c1,t.c2]].forEach(([f,c1,c2])=>{
    const by=-bH*(1-f);
    const bg=ctx.createLinearGradient(-W_*0.5,0,W_*0.5,0);
    bg.addColorStop(0,c2); bg.addColorStop(0.5,c1); bg.addColorStop(1,c2);
    ctx.fillStyle=bg;
    ctx.beginPath(); ctx.roundRect(-W_*0.5,by,W_,bH*0.11,[4]); ctx.fill();
    ctx.fillStyle='rgba(255,255,255,0.18)'; ctx.fillRect(-W_*0.48,by+1,W_*0.96,bH*0.04);
  });
  ctx.fillStyle='rgba(255,200,0,0.9)';
  ctx.font=\`bold \${Math.max(12,18*sc)}px sans-serif\`;
  ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillText('⚠',0,-bH*0.5);
}

function oColumn(W_,H_,t,sc){
  const g=ctx.createLinearGradient(-W_*0.35,0,W_*0.35,0);
  g.addColorStop(0,t.c2); g.addColorStop(0.3,t.c3); g.addColorStop(0.7,t.c1); g.addColorStop(1,t.c2);
  ctx.fillStyle=g;
  ctx.beginPath(); ctx.roundRect(-W_*0.36,-H_,W_*0.72,H_,[3,3,0,0]); ctx.fill();
  for(let i=0;i<8;i++){ctx.fillStyle='rgba(0,0,0,0.08)'; ctx.fillRect(-W_*0.36,-H_+i*(H_/8),W_*0.72,2);}
  ctx.fillStyle=t.c3; ctx.beginPath(); ctx.roundRect(-W_*0.52,-H_-H_*0.07,W_*1.04,H_*0.08,[2]); ctx.fill();
  ctx.fillStyle=t.c1; ctx.beginPath(); ctx.roundRect(-W_*0.44,-H_-H_*0.14,W_*0.88,H_*0.08,[2]); ctx.fill();
  ctx.fillStyle=t.c3; ctx.beginPath(); ctx.roundRect(-W_*0.52,0,W_*1.04,H_*0.06,[2]); ctx.fill();
}

function oSign(W_,H_,t,sc){
  const sH=H_*0.65;
  ctx.fillStyle='#aaa'; ctx.fillRect(-W_*0.05,-sH,W_*0.1,sH);
  const sg=ctx.createLinearGradient(-W_*0.5,-sH,W_*0.5,-sH*0.38);
  sg.addColorStop(0,t.c2); sg.addColorStop(1,t.c1);
  ctx.fillStyle=sg; ctx.beginPath(); ctx.roundRect(-W_*0.5,-sH,W_,-sH*0.42,[6]); ctx.fill();
  ctx.strokeStyle='rgba(255,255,255,0.3)'; ctx.lineWidth=1.5*sc;
  ctx.beginPath(); ctx.roundRect(-W_*0.46,-sH+sH*0.05,W_*0.92,-sH*0.35,[3]); ctx.stroke();
  ctx.fillStyle='#fff';
  ctx.font=\`bold \${Math.max(9,13*sc)}px Orbitron,monospace\`;
  ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillText('DY'  , 0,-sH*0.68);
  ctx.font=\`\${Math.max(6,9*sc)}px sans-serif\`;
  ctx.fillStyle='rgba(255,255,255,0.65)';
  ctx.fillText('CAMPUS',0,-sH*0.52);
}

function oStatue(W_,H_,t,sc){
  const pg=ctx.createLinearGradient(-W_*0.45,0,W_*0.45,0);
  pg.addColorStop(0,t.c2); pg.addColorStop(0.5,t.c1); pg.addColorStop(1,t.c2);
  ctx.fillStyle=pg; ctx.beginPath(); ctx.roundRect(-W_*0.44,0,W_*0.88,H_*0.18,[2,2,0,0]); ctx.fill();
  const fg=ctx.createLinearGradient(-W_*0.28,-H_,W_*0.28,0);
  fg.addColorStop(0,t.c3); fg.addColorStop(0.5,t.c1); fg.addColorStop(1,t.c2);
  ctx.fillStyle=fg;
  ctx.beginPath(); ctx.roundRect(-W_*0.25,-H_*0.74,W_*0.5,H_*0.5,[4,4,6,6]); ctx.fill();
  ctx.beginPath(); ctx.arc(0,-H_*0.82,W_*0.2,0,Math.PI*2); ctx.fill();
  ctx.save();
  ctx.strokeStyle=t.c3; ctx.lineWidth=W_*0.1; ctx.lineCap='round';
  ctx.beginPath(); ctx.moveTo(-W_*0.22,-H_*0.66); ctx.lineTo(-W_*0.55,-H_*0.95); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(W_*0.22,-H_*0.66); ctx.lineTo(W_*0.1,-H_*0.26); ctx.stroke();
  ctx.restore();
  ctx.save();
  ctx.shadowColor=t.c1; ctx.shadowBlur=18*sc;
  ctx.strokeStyle=t.c3; ctx.lineWidth=1;
  ctx.globalAlpha=0.6;
  ctx.beginPath(); ctx.arc(0,-H_*0.82,W_*0.22,0,Math.PI*2); ctx.stroke();
  ctx.restore();
}

function oHydrant(W_,H_,t,sc){
  const hH=H_*0.55;
  const bg=ctx.createLinearGradient(-W_*0.4,0,W_*0.4,0);
  bg.addColorStop(0,t.c2); bg.addColorStop(0.5,t.c1); bg.addColorStop(1,t.c2);
  ctx.fillStyle=bg;
  ctx.beginPath(); ctx.roundRect(-W_*0.4,-hH,W_*0.8,hH,[W_*0.15,W_*0.15,0,0]); ctx.fill();
  ctx.fillStyle=t.c3;
  ctx.beginPath(); ctx.ellipse(0,-hH,W_*0.35,W_*0.12,0,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.arc(0,-hH-W_*0.08,W_*0.12,0,Math.PI*2); ctx.fill();
  [-W_*0.45,W_*0.45].forEach(nx=>{
    ctx.fillStyle=t.c2;
    ctx.beginPath(); ctx.roundRect(nx>0?nx-W_*0.08:nx,(-hH*0.6-W_*0.08),W_*0.12,W_*0.16,[2]); ctx.fill();
    ctx.fillStyle='#333';
    ctx.beginPath(); ctx.ellipse(nx>0?nx:nx+W_*0.06,-hH*0.6,W_*0.06,W_*0.04,0,0,Math.PI*2); ctx.fill();
  });
  ctx.fillStyle='rgba(255,255,255,0.18)';
  ctx.beginPath(); ctx.roundRect(-W_*0.28,-hH+hH*0.08,W_*0.14,hH*0.55,[4]); ctx.fill();
}

function oBollard(W_,H_,t,sc){
  const bH=H_*0.6;
  const g=ctx.createLinearGradient(-W_*0.22,0,W_*0.22,0);
  g.addColorStop(0,t.c2); g.addColorStop(0.3,t.c3); g.addColorStop(0.7,t.c1); g.addColorStop(1,t.c2);
  ctx.fillStyle=g;
  ctx.beginPath(); ctx.roundRect(-W_*0.22,-bH,W_*0.44,bH,[W_*0.15,W_*0.15,0,0]); ctx.fill();
  [0.25,0.5,0.75].forEach(f=>{
    ctx.fillStyle='rgba(0,0,0,0.18)'; ctx.fillRect(-W_*0.22,-bH+bH*f,W_*0.44,bH*0.06);
  });
  ctx.fillStyle=t.c3;
  ctx.beginPath(); ctx.ellipse(0,-bH,W_*0.25,W_*0.1,0,0,Math.PI*2); ctx.fill();
  ctx.save();
  ctx.shadowColor=t.c1; ctx.shadowBlur=10*sc;
  ctx.strokeStyle='rgba(255,255,255,0.55)'; ctx.lineWidth=2*sc;
  ctx.beginPath(); ctx.ellipse(0,-bH*0.45,W_*0.23,W_*0.06,0,0,Math.PI*2); ctx.stroke();
  ctx.restore();
  ctx.fillStyle='rgba(255,255,255,0.15)';
  ctx.beginPath(); ctx.roundRect(-W_*0.12,-bH+bH*0.06,W_*0.08,bH*0.72,[4]); ctx.fill();
}

function spawnHit(x,y){
  const cs=['#ff4400','#ff8800','#ffcc00','#fff','#ff0044'];
  for(let i=0;i<24;i++){
    G.particles.push({
      x,y,vx:(Math.random()-.5)*14,vy:(Math.random()-.5)*14-5,
      r:Math.random()*7+2,col:cs[Math.floor(Math.random()*cs.length)],life:55,ml:55
    });
  }
}
function spawnDodge(x,y){
  for(let i=0;i<10;i++){
    G.particles.push({
      x,y,vx:(Math.random()-.5)*6,vy:-Math.random()*5-2,
      r:Math.random()*3+1,col:'#00ff88',life:32,ml:32
    });
  }
}
function drawParticles(){
  ...
  G.particles.forEach(p=>{
    p.x+=p.vx; p.y+=p.vy; p.vy+=0.28; p.vx*=0.94; p.life--;
    ctx.save();
...
function updateHUD(){
  document.getElementById('sv').innerHTML=Math.floor(G.score)+'<small style="font-size:14px;opacity:.55">m</small>';
  const h=document.getElementById('hearts');
  h.innerHTML=Array.from({length:3},(_,i)=>
    i<G.lives
      ? '<span class="heart-icon">❤️</span>'
  ...
  if(sb&&sp){
    sp.textContent=(G.speed/5.5).toFixed(1);
    sb.style.opacity=G.speed>6?'1':'0';
    const t=Math.min(1,(G.speed-5.5)/8);
    sb.style.color=t<0.5?'#ffc300':'#ff7700';
    sb.style.textShadow=\`0 0 8px \${t<0.5?'#ffc300':'#ff7700'}\`;
  }
}

function collides(ob){
  if(!ob.ssc) return false;
  const ow=60*ob.ssc*0.42, oh=110*ob.ssc*0.36;
  return Math.abs(G.playerX-ob.sx)<ow+18 && Math.abs(PLY()-ob.sy)<oh+28;
}

function loop(){
  if(!running) return;
  G.frame++; G.scroll++;
  G.score+=G.speed*0.046;

  if(G.frame%230===0){ G.speed=Math.min(19,G.speed+0.6); G.obsInterval=Math.max(32,G.obsInterval-5); }

  const lanes=LANES();
  G.playerX+=(lanes[G.lane]-G.playerX)*0.14;

  G.obsTimer++;
  if(G.obsTimer>=G.obsInterval){
    G.obsTimer=0; spawnObs();
    if(G.speed>8&&Math.random()<0.32) setTimeout(()=>{if(running)spawnObs();},280);
  }

  G.obs.forEach(ob=>{
    ob.depth+=G.speed*0.00088;
    const lanes2=LANES();
    ob.wx=lanes2[ob.lane];
  });
  G.obs=G.obs.filter(o=>o.depth<1.4);

  if(G.inv<=0){
    for(const ob of G.obs){
      if(ob.depth>0.55&&ob.depth<0.98&&collides(ob)){
        G.lives--; G.inv=95;
        spawnHit(G.playerX,PLY()-40);
        ob.depth=99;
        G.shakeT=18; G.shakeA=14;
  ...
  if(G.shakeT>0){
    G.shakeT--; G.shakeA*=0.82;
    shell.style.transform=\`translate(\${(Math.random()-.5)*G.shakeA}px,\${(Math.random()-.5)*G.shakeA}px)\`;
  } else { shell.style.transform=''; }

  ctx.clearRect(0,0,W,H);
  drawBG();
  [...G.obs].sort((a,b)=>a.depth-b.depth).forEach(ob=>drawObs(ob));
  drawPlayer();
  drawParticles();
  updateHUD();

  ctx.save(); ctx.globalAlpha=0.022;
  for(let y2=0;y2<H;y2+=4){ ctx.fillStyle='#000'; ctx.fillRect(0,y2,W,2); }
  ctx.restore();
...
function spawnObs(){
  ...
  G.obs.push({wx:LANES()[lane],lane,depth:0.03,type,sx:0,sy:0,ssc:0.05});
}

function startGame(){
  document.getElementById('menuScreen').classList.add('off');
  document.getElementById('goScreen').classList.add('off');
...
function endGame(){
  ...
  document.getElementById('gsFinal').textContent=Math.floor(G.score)+'m';
  document.getElementById('goScreen').classList.remove('off');
  document.getElementById('lbBox2').innerHTML='';
}
...
window.addEventListener('touchend',e=>{
  ...
  if(Math.abs(dx)>20){
    G.lane=dx<0?Math.max(0,G.lane-1):Math.min(2,G.lane+1);
    spawnDodge(G.playerX,PLY());
  }
},{passive:true});
window.addEventListener('keydown',e=>{
  if(!running) return;
  if(e.key==='ArrowLeft'||e.key==='a'){G.lane=Math.max(0,G.lane-1);spawnDodge(G.playerX,PLY());}
  if(e.key==='ArrowRight'||e.key==='d'){G.lane=Math.min(2,G.lane+1);spawnDodge(G.playerX,PLY());}
});
let md=false,mx=0;
window.addEventListener('mousedown',e=>{md=true;mx=e.clientX;});
window.addEventListener('mouseup',e=>{
  if(!md||!running){md=false;return;} md=false;
  const dx=e.clientX-mx;
  if(Math.abs(dx)>20) G.lane=dx<0?Math.max(0,G.lane-1):Math.min(2,G.lane+1);
});

document.getElementById('btnStart').addEventListener('click',startGame);
document.getElementById('btnSave').addEventListener('click',()=>{
  saveLB(document.getElementById('pname').value,Math.floor(G.score));
  startGame();
});
document.getElementById('btnLB1').addEventListener('click',()=>{
  const w=document.getElementById('lbBox');
  w.style.display=w.style.display==='block'?'none':'block';
  if(w.style.display==='block') renderLB('lbBox');
});
document.getElementById('btnLB2').addEventListener('click',()=>{
  const w=document.getElementById('lbBox2');
  w.style.display=w.style.display==='block'?'none':'block';
  if(w.style.display==='block') renderLB('lbBox2');
});

(function(){
  ctx.fillStyle='#040c1a'; ctx.fillRect(0,0,W,H);
...
<\/script>
        ...
        doc.open();
        doc.write(gameSource);
        doc.close();
    };

    window.stopCampusRunGame = function() {
