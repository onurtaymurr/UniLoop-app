...
// Olay Dinleyicisi (Event Listener) Kısayolu - Modül olduğu için doğrudan çalışır
const bind = (id, event, callback) => { 
    const el = document.getElementById(id); 
    if (el) {
        el.addEventListener(event, callback); 
    } else {
        console.warn(`Uyarı: '${id}' ID'li buton HTML'de bulunamadı.`);
    }
};

// --- SİSTEM HAFIZASI (GLOBAL DEĞİŞKENLER) ---
window.userProfile = { 
    uid: "", name: "", surname: "", email: "", university: "", avatar: "👨‍🎓", faculty: "" 
};
...
// ============================================================================
// 1. GİRİŞ, KAYIT, DOĞRULAMA EKRANI GEÇİŞLERİ
// ============================================================================
...
bind('login-btn', 'click', async () => {
    ...
    try {
        ...
        if (docSnap.exists() && docSnap.data().isVerified === false) {
            ...
            if(vCard) vCard.style.display = 'block';
            btn.innerText = originalText; btn.disabled = false;
            return alert("Hesabınızı henüz kod ile onaylamadınız. Lütfen kodu girin.");
        }
...
onAuthStateChanged(auth, async (user) => {
    if (user) { 
        ...
        // ÇIKIŞ YAPILMIŞ DURUM YA DA İLK GİRİŞ EKRANI
        if(authScreen && appScreen) {
            appScreen.style.display = 'none';
            authScreen.style.display = 'flex';
            document.getElementById('login-card').style.display = 'block';
